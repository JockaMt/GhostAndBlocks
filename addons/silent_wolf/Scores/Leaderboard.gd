...
const ScoreItem = preload("res://Misc/LeaderboardItem.tscn")
...
func add_item(player_name, score):
	...
	list_index += 1
	item.get_node("Texts/Nickname").text = str(list_index) + str(". ") + player_name
	item.get_node("Texts/Score").text = score
	item.margin_top = list_index * 100
	$"Panel-2/ScrollContainer/Leaderboard".add_child(item)


func add_no_scores_message():
	var item = $"Panel-2/MensageContainer/Label"
	item.text = "No scores yet!"
	$"Panel-2/MensageContainer".show()
	item.margin_top = 135
...
func hide_message():
	$"Panel-2/MensageContainer".hide()


func clear_leaderboard():
	var score_item_container = $"Panel-2/ScrollContainer/Leaderboard"
